...
import com.atguigu.sparkmall.util.bean.DataModel.{CityInfo, ProductInfo, UserInfo, UserVisitAction}
import common.ConfigurationUtil
import util.{RandomDate, RandomNumUtil, RandomOptions}
...
object MockOffline {
    ...
    // 动作
    val actionOpts = RandomOptions(("search", 20), ("click", 60), ("order", 6), ("pay", 4), ("quit", 5))
    // session 数量
    ...
    def main(args: Array[String]): Unit = {
      ...
      import spark.implicits._
        ...
        val userInfoDF: DataFrame = sc.makeRDD(userInfoData).toDF
        val productInfoDF: DataFrame = sc.makeRDD(productInfoData).toDF
        val cityInfoDF: DataFrame = sc.makeRDD(cityInfoData).toDF
